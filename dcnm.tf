...
provider "dcnm" {
  ...
  username = data.vault_generic_secret.mel-dcnm.data["username"]
  # cisco-dcnm password
  # password = var.dcnm_password
  password = data.vault_generic_secret.mel-dcnm.data["password"]
  # cisco-dcnm url
  # url      = var.dcnm_url
  url = data.vault_generic_secret.mel-dcnm.data["url"]
  insecure = true
}
...
resource "dcnm_network" "tfcb-net-2" {
  ...
  description     = "L3 VXLAN Network #2. Created by Terraform Cloud 4"
  vrf_name        = dcnm_vrf.tfcb-vrf-1.name
  vlan_id         = 1602
...
# ### Create Distributed Port Group Name
# locals {
